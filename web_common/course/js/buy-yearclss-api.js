$(function($){
	...
	$(".kecheng").click(function(){
		//var lessonaid = $("#lessonaid").val();
        var lessonaid = $(this).children().val();
       // alert(lessonaid)
		//window.location.href=addressUrl+"index.php/Home/Course/redirectUrl/course_id/"+lessonaid;
	});

    //用户端
	$(".kec").click(function(){
		//var lessonbid = $(".lessonbid").val();
        var lessonbid=$(this).children().val();
		window.location.href=addressUrl+"index.php/Home/Course/redirectUrl/course_id/"+lessonbid;
	});
	//已经购买评价页调数据
	var aaa = $(".lessId").val();
	...
	$.ajax({
		...
		success:function(data){
			...
			for(var i=0;i<data.info.length;i++){
				console.log(data.info[i].geval_content);
				var star = '';
...
//				}
				if(data[i].status==0){
					str +='<ul class="kecheng" data-name='+data[i].id+'><a href="" class="zhibo"><li class="yuan">'+ (i+1) +'</li><li class="mulu-title dot">'+ data[i].title +'</li><li class="mulu-time">未开课</li></a></ul>';
				}else if(data[i].status==1){
					str +='<ul class="kecheng" data-name='+data[i].id+'><a href="" class="zhibo"><li class="yuan">'+ (i+1) +'</li><li class="mulu-title dot">'+ data[i].title +'</li><li class="mulu-time">开课中</li></a></ul>';
				}else if(data[i].status==2){
					str +='<ul class="kecheng" data-name='+data[i].id+'><a href="" class="zhibo"><li class="yuan">'+ (i+1) +'</li><li class="mulu-title dot">'+ data[i].title +'</li><li class="mulu-time">已结课</li></a></ul>';
				}
			}
			$(".show .bg").html(str);
			changeClo($(".yuan"));
...
//		 	alert("目录页出错！");
	...
	$.ajax({
		...
		success:function(data){
			...
			$(".pho").css({background:"url("+arrxqstar[0].cover_url+")",
				backgroundSize:"100% 100%"
			})

			if(arrxqstar[0].cover_url == null || arrxqstar[0].cover_url == "null" || arrxqstar[0].cover_url.length < 0){
			   console.log("-----------------------")
			   console.log(arrxqstar[0].cover_url)
			   console.log("-----------------------")
			}else{
			   $(".head").css({background:"url("+arrxqstar[0].cover_url+")",backgroundSize:"100% 100%"})
			}
